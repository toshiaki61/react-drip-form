...
import { Consumer } from './dripFormContext';
import type {
  $WrappedComponent,
  DFContext,
  ErrorMessage,
...
const dripFormField = (fieldType: FieldType = 'text', options: FieldOptions = {}) => (
  (WrappedComponent: $WrappedComponent<*, *>) => ((wrappedProps: FieldProps) => (
    <Consumer>
      {(v) => {
        class DripFormField extends Component<FieldProps, *> {
          ...
          constructor(props: FieldProps) {
            super(props);
            console.log('dripFormField', props);
            const { context } = props;
        ...
        return <DripFormField {...wrappedProps} context={v.context} />;
      }}
    </Consumer>
  ))
);
